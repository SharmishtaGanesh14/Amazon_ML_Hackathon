...
class MultimodalMLP(nn.Module):
    def __init__(self, input_dim, dropout=0.2):
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(1024, 512), nn.BatchNorm1d(512), nn.ReLU(), nn.Dropout(dropout*0.8),
            nn.Linear(512, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(dropout*0.6),
            nn.Linear(256, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Dropout(dropout*0.4),
...
def train_multimodal_cnn(X_train, y_train, X_val, y_val, epochs=60, lr=5e-4):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_size = min(512, len(X_train)//10) if device.type=='cuda' else min(128,len(X_train)//10)
    scaler = RobustScaler()
    X_train_scaled = scaler.fit_transform(X_train).astype(np.float32)
    X_val_scaled = scaler.transform(X_val).astype(np.float32)
    train_loader = DataLoader(TensorDataset(torch.from_numpy(X_train_scaled), torch.from_numpy(y_train)),
                              batch_size=batch_size, shuffle=True, pin_memory=(device.type=='cuda'), drop_last=True)
    X_val_tensor = torch.from_numpy(X_val_scaled).to(device)
    y_val_tensor = torch.from_numpy(y_val).to(device)

    model = MultimodalMLP(X_train_scaled.shape[1]).to(device)
    loss_fn = SMAPELoss()
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=0.001, eps=1e-8)
...
def create_ensemble_predictions(lgb_model, cnn_model, X_val, y_val=None):
    ...
    X_scaled = cnn_model.feature_scaler.transform(X_val).astype(np.float32)
    with torch.no_grad():
        cnn_pred = cnn_model(torch.from_numpy(X_scaled).to(device)).cpu().numpy()
    meta_features = np.column_stack([lgb_pred, cnn_pred])
    stacker = Ridge(alpha=10.0)
...
# ── Main Pipeline ─────────────────────────────────────────────
def main():
    dataset_dir = "./dataset"  # ✅ fixed path
    ...
    output_df.to_csv("multimodal_predictions.csv", index=False)
    print("Predictions saved to multimodal_predictions.csv")
